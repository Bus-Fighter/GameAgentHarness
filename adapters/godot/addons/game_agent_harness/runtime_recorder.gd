...
func _ready() -> void:
	...
	# Default runtime capture to OFF until the dashboard explicitly enables it.
	# This avoids viewport-capture overhead/crashes during autoload startup.
	if not ProjectSettings.has_setting("game_agent_harness/runtime_capture_enabled"):
		ProjectSettings.set_initial_value("game_agent_harness/runtime_capture_enabled", false)
		ProjectSettings.set_setting("game_agent_harness/runtime_capture_enabled", false)
...
func _take_snapshot() -> void:
	...
	var image := _capture_viewport()
	if image == null:
		_log_warning("snapshot failed: could not capture viewport")
		return
